...
### Calculation of profitability measures 

Using the **subsample of your industry with all quarters**, create new columns for the following variables as measures of firm performance and profitability:
...
Using annual fiscal data (selecting rows with fiscalmonth=12), and the year 2022:
...
Select 3 variables/ratios with the strongest correlation with quarterly returns and do the following:
...
-   Design and run a first version of a multiple regression model to examine whether your selected factors can explain/predict quarerly stock returns.

-   For each independent variable you have to check for outliers (extreme values) and if it is necessary do the corresponding adjustments with winsorization to avoid unreliable results in your regression model
...
-   Once you corrected your variables for outliers and address possible multicollienarity problems, run a final multiple regression model
